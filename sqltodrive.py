...
def filestobackup(fdir):
    if not fdir:
        up_log(datetime.strftime(datetime.now(), "%Y.%m.%d %H:%M:%S") + ' ' + backupdir + ' > ' + todir + ' (please set source folder, use -d path_to_folder/folder_name)')
    else:
        filestobackup = []
        backupfiles = []

        if not os.path.isdir(fdir):
            name_file = fdir.split('/')
            fdir = fdir.replace(name_file[len(name_file) - 1], '')
            fdir = fdir.lstrip('/').rstrip('/')
            backupfiles.append(name_file[len(name_file) - 1])
        else:
        ...
        if not backupfiles:
            up_log(datetime.strftime(datetime.now(), "%Y.%m.%d %H:%M:%S") + ' ' + backupdir + ' > ' + todir + ' (folder empty)')
        else:
            service = drive_service()
